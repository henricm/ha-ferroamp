...
async def test_setting_ehub_sensor_values_via_mqtt_message(hass, mqtt_mock):
    ...
    assert state.state == "1228.35821955083"
    assert state.attributes == {
        "device_class": "energy",
    ...
    assert state.state == "1366.37360237528"
    assert state.attributes == {
        "device_class": "energy",
    ...
    assert state.state == "1242.4428329425"
    assert state.attributes == {
        "device_class": "energy",
    ...
    state = hass.states.get(
        "sensor.ferroamp_available_three_phase_active_current_for_load_balancing"
    )
    assert state.state == "29.05"
    assert state.attributes == {
        "device_class": "current",
...
async def test_only_adding_load_balancing_sensors_if_present_in_message(
    hass, mqtt_mock
):
    config_entry = MockConfigEntry(
        domain=DOMAIN,
...
async def test_only_adding_battery_specific_sensors_if_present_in_mqtt_message(
    hass, mqtt_mock
):
    config_entry = MockConfigEntry(
        domain=DOMAIN,
...
async def test_trim_part_no_from_esm_id(hass, mqtt_mock):
    ...
    sensor = hass.data[DOMAIN][DATA_DEVICES][config_entry.unique_id][
        "ferroamp_esm_12345678"
    ][entity.unique_id]
...
async def test_migrate_old_esm_entities(hass, mqtt_mock):
    ...
    entity = er.async_get_or_create(
        "sensor",
        DOMAIN,
        "ferroamp_esm_ES01Z000012345678 -soh",
        config_entry=config_entry,
    )
...
async def test_setting_eso_sensor_values_via_mqtt_message(hass, mqtt_mock):
    ...
    assert state.state == "684.751700573056"
    assert state.attributes == {
        "device_class": "energy",
    ...
    assert state.state == "614.870966532778"
    assert state.attributes == {
        "device_class": "energy",
...
async def test_setting_sso_sensor_values_via_mqtt_message(hass, mqtt_mock):
    ...
    assert state.state == "234.310112298056"
    assert state.attributes == {
        "device_class": "energy",
...
async def test_migrate_old_sso_entities(hass, mqtt_mock):
    ...
    entity = er.async_get_or_create(
        "sensor",
        DOMAIN,
        "ferroamp_sso_PS00990-A02-S12345678-upv",
        config_entry=config_entry,
    )
...
async def test_update_options(hass, mqtt_mock):
    ...
    assert entity is not None
    sensor = hass.data[DOMAIN][DATA_DEVICES][config_entry.unique_id]["ferroamp_esm_1"][
        entity.unique_id
    ]
    ...
    assert entity is not None
    sensor = hass.data[DOMAIN][DATA_DEVICES][config_entry.unique_id]["ferroamp_ehub"][
        entity.unique_id
    ]
    ...
    assert entity is not None
    sensor = hass.data[DOMAIN][DATA_DEVICES][config_entry.unique_id]["ferroamp_eso_1"][
        entity.unique_id
    ]
    ...
    assert entity is not None
    sensor = hass.data[DOMAIN][DATA_DEVICES][config_entry.unique_id]["ferroamp_sso_1"][
        entity.unique_id
    ]
    assert isinstance(sensor, VoltageFerroampSensor)
    assert sensor._interval == 20
    ...
    assert sensor.state == 1228.3582195508334
...
async def test_3phase_always_increasing_unknown_value(hass, mqtt_mock):
    ...
    assert sensor.state == 662.4200000000001
